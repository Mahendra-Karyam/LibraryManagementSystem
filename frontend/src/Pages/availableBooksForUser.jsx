...
/*
Each Book object returned from the API is expected to look like:
  {
    _id: string,
    Title: string,
    Author: string,
...
export default function AvailableBooksForUser() {
  ...
  /*
    borrowedBooksRef must be attached to a <div> using ref.
    What is ref?
  ...
  */
  ...
  const fetchUser = async () => {
    // No need to read the token manually — the shared `api` instance's
    // interceptor (src/api/axios.js) attaches it to every request.
    try {
  ...
  const handleBorrow = async (id, bookTitle) => {
    try {
      await api.put(`/books/borrow/${id}`, { BorrowerName: userName });
